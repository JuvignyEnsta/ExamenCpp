 ...
 */
std::vector<polynome<double>> construire_base_ortho_polynomiale(int dim, const produit_scalaire_base& dot )
{
    std::vector<polynome> base;
    base.reserve(dim);
    // Construit le polynome de degré zéro p0(x) = 1
    base.emplace_back(polynome::canonique(0));
    // Normalisation premier vecteur
    base.back() *= std::sqrt(1./dot(base.back(),base.back()));
    // Algorithme de Gram-Schmidt modifié :
    for ( int i = 1; i < dim; ++i )
    {
        // Construit le polynome de degré i pi(x) = x^i
        polynome q = polynome::canonique(i);
        for ( int j = 0; j < i; ++j )
            q -= dot(q,base[j])*base[j];
        q *= std::sqrt(1./dot(q,q));
        base.emplace_back(q);
    }
    return base;
}
...
int main()
{
    ...
    auto dotL = produit_legendre(5);// Produit scalaire int_{-1}^{1}P(x)Q(x)dx
    ...
    auto dot = produit_scalaire_tchebychev(5);// Produit scalaire int_{-1}^{1}P(x)Q(x)/sqrt(1-x²) dx
    ...
    std::cout << "VERIFICATION\n"
              << "------------\n";
    std::cout << "Les racines des differents polynomes (a 10^-3 pres a cause du calcul integrale peu precis) : \n";
    for ( int n = 1; n < 5; ++n )
    {
        for ( int i = 1; i <= n; ++i )
        {
